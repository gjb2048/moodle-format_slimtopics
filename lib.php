 ...
 */
function callback_slimtopics_uses_sections() {
    return true;
}
 ...
 * @return bool Returns true
 */
function callback_slimtopics_load_content(&$navigation, $course, $coursenode) {
 ...
 * @return string
 */
function callback_slimtopics_definition() {
 ...
 * @return string
 */
function callback_slimtopics_request_key() {
    return 'topic';
}

function callback_slimtopics_get_section_name($course, $section) {
    // We can't add a node without any text
    if (!empty($section->name)) {
        return format_string($section->name, true, array('context' => get_context_instance(CONTEXT_COURSE, $course->id)));
    } else if ($section->section == 0) {
        return get_string('section0name', 'format_slimtopics');
    } else {
 ...
 * @return stdClass
 */
function callback_slimtopics_ajax_support() {
 ...
 * @return moodle_url
 */
function callback_slimtopics_get_section_url($courseid, $sectionnum) {
